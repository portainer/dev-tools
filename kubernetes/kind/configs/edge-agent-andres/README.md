...
1. Update paths in `kind.yaml`
2. Create the cluster: `REPO_ROOT/kubernetes/kind/setup.sh create edge-agent-andres`
...
`CONTEXT=edge-agent-andres REPO_ROOT/kubernetes/kind/edge-agent-setup.sh EDGE_ID EDGE_KEY`
With my example:
```sh
